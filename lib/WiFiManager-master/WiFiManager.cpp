...
int WiFiManager::connectWifi(String ssid, String pass) {
  ...
  if (ssid != "") {
    ...
    if (WiFi.SSID()) {
      ...
      DEBUG_WM("WifiStation Disconnected");
      WiFi.begin();
      DEBUG_WM("Wifi Begin");
    } else {
...
uint8_t WiFiManager::waitForConnectResult() {
  if (_connectTimeout == 0) {
    DEBUG_WM (F("Wifi Wait For Connect Result"));
    uint8_t connRes;
    for (int i=0; i<11; i++){
       DEBUG_WM (F("FOR"));
       if ( (connRes = WiFi.waitForConnectResult()) == WL_CONNECTED ) break;
    }
    return connRes;//WiFi.waitForConnectResult();
  } else {
